...
export const ReceiptCard: React.FC<ReceiptCardProps> = ({
  ...
  return (
    <div
      ...
      className="group relative receipt-paper rounded-xl p-5 border border-white/10 transition-all duration-300 hover:-translate-y-1 hover:border-sky-500/40 hover:shadow-receipt cursor-pointer flex flex-col justify-between"
    >
      ...
      <div className="pt-3 border-t border-white/5 flex items-center justify-between">
        ...
        <div className="w-20 h-4 barcode-strip rounded" />

        <div className="flex items-center gap-2">
          {onFindConnections && (
            <button
              onClick={(e) => onFindConnections(receipt, e)}
              className="text-[11px] font-mono px-2.5 py-1 rounded bg-sky-500/10 hover:bg-sky-500/20 text-sky-300 border border-sky-500/30 transition-all"
            >
              Connect
            </button>
          )}
